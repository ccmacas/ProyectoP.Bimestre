...
public class Operaciones {
    ...
    public double caja;
//Metodo para acceder a las diversas opciones que tiene nuestro MENU
    ...
    public void menu(int n, ArrayList<Producto> productos, ArrayList<Personal> personal) {
        int op, cantidad = 0,opcion =0;
       // while(opcion!=9){
        switch (n) {
            //Caso 1
            case 1:
                //Si el usuario digita 1 se llama al metodo registrarNuevoTrabajador
            ...
            case 3:
                ...
                do {
                    cantidad = errorNumero("Ingrese la  Cantidad que desea del Producto");
                } while (cantidad == -1);
                //Se llama al metodo venderProducto.
...
//Metodo para mostrar los clientes registrados  que recibe como parametro un arrayList de tipo Personal
    ...
    public void mostrarClientes(ArrayList<Personal> personal) {
        ...
        if (!personal.isEmpty()) {
            ...
            para obtener el tamaño usamos un metodo de la clase Arraylist que es el .size() q es un metodo 
            que devuelve un valor entero*/
             
            for (int i = 0; i < personal.size(); i++) {
                /*A nuestra variable cliente le almancenamos los valores que obtiene
...
//Metodo  para mostar los productos
    ...
    public double venderProducto(ArrayList<Producto> productos, int respuesta, int cantidadP) {
        ...
        if (productos.get(respuesta - 1).isDisponible()) {
            ...
            if (catalogo instanceof Computadoras && productos.get(respuesta - 1).getCantidStock() >= cantidadP) {
                //Si se cumple establece la nueva cantidad de stock
                productos.get(respuesta - 1).setCantidStock(productos.get(respuesta - 1).getCantidStock() - cantidadP);
            ...
            } else if (productos.get(respuesta - 1).getCantidStock() >= cantidadP) {
                JOptionPane.showMessageDialog(null, "Venta Realizada con exito");
                productos.get(respuesta - 1).setCantidStock(productos.get(respuesta - 1).getCantidStock() - cantidadP);
                return caja += cantidadP * productos.get(respuesta - 1).getPrecioUnitar();
...
//Metodo modificar que recibe parametros un arraylist, respueta del usuario, y una varialbe tipo catalogo
    ...
    public double Modificar(ArrayList<Producto> productos, int respuesta, Producto catalogo) {
        ...
        do {
            mo = errorNumero("Desea Modificar alguna parte del equipo:\n 1.Si\n2.No");
        } while (mo == -1 || mo > 2);
        ...
        if (mo == 1) {
            do {
                ...
                n = errorNumero("Qué desea Modificar \n1.Disco Duro\n 2.Memoria Ram\n 3.Procesador");
            } while (n == -1 || n > 3);
            ...
            switch (n) {
                ...
                case 1: {
                    do {
                        opcion = errorNumero("1.500 GB  $160 \n2.1000 GB  $200\n3. 2000 GB $260");
                    } while (opcion == -1 || opcion > 3);
                    presActual = (productos.get(respuesta - 1).getPrecioUnitar() + preciosD[opcion - 1]);
                ...
                case 2: {
                    ...
                    do {
                        opcion = errorNumero("1.2GB  $130\n2.4GB  $250 \n3.8Gb $450");
                    } while (opcion == 1 || opcion > 3);
                    presActual = productos.get(respuesta - 1).getPrecioUnitar() + precioRam[opcion - 1];
                ...
                case 3:
                    ...
                    do {
                        opcion = errorNumero("1.Core i3  $200\n2.Core i5  $350\n3.Core i7  $450");
                    } while (opcion == -1 || opcion > 3);
                    presActual = productos.get(respuesta - 1).getPrecioUnitar() + preciosD[opcion - 1];
                    productos.get(respuesta - 1).setPrecioUnitar(presActual + precioProce[opcion - 1]);
            ...
            return productos.get(respuesta - 1).getPrecioUnitar();
        }
    }
        ...
        for (Personal salarios : personal) {
            if (salarios instanceof Empleado) {
                sal = sal + (" Nombre: " + salarios.getNombre() + " Apellido:" + salarios.getApellido()
                        + " Cargo:" + ((Empleado) salarios).getCargo() + " Salario:" + ((Empleado) salarios).getSueldo());
            }
        }
    ...
    public int errorNumero(String mensaje) {
        int numero = 0;
        try {
